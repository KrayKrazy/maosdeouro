  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-2xl font-serif text-white">Cat�logo de Produtos</h2>
          <p className="text-sm text-gray-400 mt-1">Gerencie os itens, varia��es e pre�os por metro quadrado.</p>
        </div>
      ...
      {loading ? (
        <div className="text-center py-20 text-[#D4AF37] animate-pulse">Carregando cat�logo...</div>
      ) : (
          ...
          {products.map(p => (
            <div key={p.id} className="bg-[#0a0a0a] border border-white/10 rounded-xl overflow-hidden flex flex-col md:flex-row">
              ...
              <div className="p-6 flex-1">
                ...
                <div className="mt-6">
                  <h4 className="text-xs uppercase tracking-widest text-gray-500 mb-3">Variantes & Pre�os (R$ / {p.unit})</h4>
                    ...
                    {p.variants && p.variants.map((v: any) => (
                      <div key={v.id} className="p-3 flex items-center justify-between hover:bg-white/5 transition-colors">
                        <div className="flex flex-col">
                          <span className="text-sm text-gray-300 font-medium">{v.vidro || v.aluminio || v.linha || v.modelo || 'Padr�o'}</span>
